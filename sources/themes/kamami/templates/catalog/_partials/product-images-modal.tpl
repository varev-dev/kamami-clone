              ...
              <ul class="product-images js-modal-product-images">
                {foreach from=$product.images item=image}
                  <li class="thumb-container js-thumb-container">
                    <img data-image-large-src="{$image.bySize.large_default.url}" class="thumb js-modal-thumb"
                      src="{$image.bySize.medium_default.url}" {if !empty($image.legend)} alt="{$image.legend}"
                      title="{$image.legend}" {else} alt="{$product.name}" 
                      {/if} width="195" height="148">
                  </li>
                {/foreach}
              </ul>
            </div>
